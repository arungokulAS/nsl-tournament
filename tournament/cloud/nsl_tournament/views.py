...
def admin_login_view(request):
    ...
    # --- Admin Schedule Pre-Quarter View ---

        # --- Admin Schedule Pre-Quarter View ---
    def admin_schedule_pre_quarter_view(request: HttpRequest) -> HttpResponse:
        lock_obj, _ = TeamsLock.objects.get_or_create(pk=1)
        prev_round_finished = getattr(lock_obj, 'qualifier_finished', False)
        schedule_locked = getattr(lock_obj, 'pre_quarter_schedule_locked', False)
        round_finished = getattr(lock_obj, 'pre_quarter_finished', False)
        schedule = getattr(lock_obj, 'pre_quarter_schedule', [])
        messages_list = []
        if not prev_round_finished:
            messages_list.append('Cannot schedule Pre-Quarter until Qualifier is finished.')
            return render(request, 'admin_schedule_pre_quarter.html', {
                'schedule': [],
                'schedule_locked': False,
                'round_finished': False,
                'messages': messages_list,
            ...
            if password != ADMIN_PASSWORD:
                ...
                if action == 'generate_schedule' and not schedule_locked:
                    ...
                    for group in group_names:
                        ...
                        group_teams = sorted(group_teams, key=lambda t: getattr(t, 'points', 0), reverse=True)
                        qualified.extend([t.team_name for t in group_teams[:4]])
                    best_losers = getattr(lock_obj, 'best_losers', [])
                    pre_quarter_teams = qualified + best_losers
                    matches = []
                    for i in range(0, len(pre_quarter_teams), 2):
                        if i+1 < len(pre_quarter_teams):
                            matches.append({'match': f'{pre_quarter_teams[i]} vs {pre_quarter_teams[i+1]}'})
                    for idx, match in enumerate(matches):
                        match['court'] = f'Court {idx % 4 + 1}'
                        match['status'] = 'Scheduled'
                    lock_obj.pre_quarter_schedule = matches
                    lock_obj.pre_quarter_schedule_locked = True
                    lock_obj.save()
                    schedule = matches
                    messages_list.append('Pre-Quarter schedule generated and locked.')
                elif action == 'delete_schedule' and not schedule_locked:
                    lock_obj.pre_quarter_schedule = []
                    lock_obj.save()
                    schedule = []
                    messages_list.append('Pre-Quarter schedule deleted.')
                elif action == 'finish_round' and schedule_locked and not round_finished:
                    for match in schedule:
                        match['status'] = 'Completed'
                    lock_obj.pre_quarter_finished = True
                    lock_obj.save()
                    round_finished = True
                    messages_list.append('Pre-Quarter finished and archived.')
        return render(request, 'admin_schedule_pre_quarter.html', {
            'schedule': schedule,
            'schedule_locked': getattr(lock_obj, 'pre_quarter_schedule_locked', False),
            'round_finished': getattr(lock_obj, 'pre_quarter_finished', False),
    ...
    # --- Tournament Teams Public Read-Only View ---
    def tournament_teams_view(request: HttpRequest) -> HttpResponse:
        teams = Team.objects.all().order_by('created_at')
        # No POST, no edit, strict read-only
        return render(request, 'tournament_teams.html', {'teams': teams})

    # --- Tournament Groups Public Read-Only View ---
    def tournament_groups_view(request: HttpRequest) -> HttpResponse:
        ...
        for group in group_names:
            ...
            groups.append({'name': group, 'teams': group_teams})
        # No POST, no edit, strict read-only
        return render(request, 'tournament_groups.html', {'groups': groups})

    # --- Tournament Schedule Public Read-Only View ---
    def tournament_schedule_view(request: HttpRequest) -> HttpResponse:
        lock_obj, _ = TeamsLock.objects.get_or_create(pk=1)
        # Collect all schedules from each phase
        group_schedule = getattr(lock_obj, 'group_schedule', [])
        qualifier_schedule = getattr(lock_obj, 'qualifier_schedule', [])
        pre_quarter_schedule = getattr(lock_obj, 'pre_quarter_schedule', [])
        knockout_schedule = getattr(lock_obj, 'knockout_schedule', [])
        # Combine all schedules for display
        schedules = {
            'Group Stage': group_schedule,
            'Qualifier': qualifier_schedule,
            'Pre-Quarter': pre_quarter_schedule,
            'Knockout': knockout_schedule,
        }
        # No POST, no edit, strict read-only
        return render(request, 'tournament_schedule.html', {'schedules': schedules})

    # --- Tournament Live Public Read-Only View ---
    def tournament_live_view(request: HttpRequest) -> HttpResponse:
        lock_obj, _ = TeamsLock.objects.get_or_create(pk=1)
        # Use latest schedule with status 'Active' or 'Scheduled'
        live_matches = []
        for phase in ['group_schedule', 'qualifier_schedule', 'pre_quarter_schedule', 'knockout_schedule']:
            schedule = getattr(lock_obj, phase, [])
            for match in schedule:
                if match.get('status') in ['Active', 'Scheduled']:
                    live_matches.append(match)
        # No POST, no edit, strict read-only
        return render(request, 'tournament_live.html', {'live_matches': live_matches})

    # --- Tournament Results Public Read-Only View ---
    def tournament_results_view(request: HttpRequest) -> HttpResponse:
        ...
        # No POST, no edit, strict read-only
        return render(request, 'tournament_results.html', {'results': results})
